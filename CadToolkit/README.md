...
一个命令 `CC` 呼出分组面板，23 个内置工具 + 无限自定义扩展，配置文件随插件一起迁移。
...
| 配置维护 | `CT_CONFIGMAINTAIN` | 打开当前 CadToolkit.ini、打开配置目录、进入配置体检，并可自动修复缺项。自动修复前会备份原配置。 |
| 规范中心 | `CT_STANDARDCENTER` | 统一进入图层规范、文字规范、配置维护和配置体检。后续新增规范功能也会优先放在这里。 |
...
运行 `CT_STANDARDCENTER` 可以统一进入图层规范、文字规范、配置维护和配置体检。运行 `CT_CONFIGMAINTAIN` 可以打开当前 CadToolkit.ini、打开配置目录、进入配置体检，并在确认后自动修复缺项；自动修复前会备份原配置。

也可以运行 `CT_CONFIGCHECK` 或面板左下角齿轮按钮直接检查 `CadToolkit.ini` 是否缺少基础项、官方命令、必要 section，是否存在映射目标缺失或标准行格式错误。
