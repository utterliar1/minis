...
Assert-Match 'batch plot dialog selects current cad validator first' $dialogs 'GetCadPlotSettingsValidatorTypeNames'
Assert-Match 'batch plot dialog prefers zwcad style validator in zwcad host' $dialogs 'ZwSoft\.ZwCAD\.DatabaseServices\.PlotSettingsValidator[\s\S]*GrxCAD\.DatabaseServices\.PlotSettingsValidator'
...
Assert-Match 'batch plot dialog chooses available current cad style' $dialogs 'SelectAvailablePlotStyle\(cmbStyle,\s*PlotStyle\)'
...
Assert-Match 'batch plot waits for pdf writer to finish before validation' $batchPlotCommands 'WaitForValidPdfFile\(outputPath\)'
Assert-Order 'batch plot waits for pdf before reporting invalid output' $batchPlotCommands 'WaitForValidPdfFile(outputPath)' 'BatchPlot output is not a valid PDF'
...
Assert-Match 'api plot computes mm expanded frame before setting window' $batchPlotCommands 'BatchPlotFrame\s+plotFrame\s*=\s*ExpandBatchPlotFrameByMarginMm\(frame,\s*settings\)'
Assert-Match 'api plot window uses mm expanded frame' $batchPlotCommands 'CreateExtents2d\(plotFrame\)'
...
Assert-Match 'api plot binds device before resolving media list' $batchPlotCommands 'BindPlotDeviceForMediaLookup\(validator,\s*plotSettings,\s*deviceName\)[\s\S]*ResolveCanonicalMediaName\(validator,\s*plotSettings,\s*settings\.PaperName\)'
...
Assert-Match 'api plot computes explicit scale from paper and margin' $batchPlotCommands 'BuildBatchPlotScaleInput\(frame,\s*settings\)'
Assert-Match 'api plot uses custom print scale when explicit scale is available' $batchPlotCommands 'SetCustomPrintScale'
Assert-Match 'api plot falls back to scale to fit when explicit scale is unavailable' $batchPlotCommands 'ScaleToFit'
Assert-Match 'api plot logs geometry diagnostics' $batchPlotCommands 'LogBatchPlotGeometry\("BatchPlot API geometry"'
...
Assert-Match 'batch plot treats begin generate graphics as optional' $batchPlotCommands 'SafeInvokeOptionalMethod\(engine,\s*"BeginGenerateGraphics"'
Assert-Match 'batch plot treats end generate graphics as optional' $batchPlotCommands 'SafeInvokeOptionalMethod\(engine,\s*"EndGenerateGraphics"'
Assert-Match 'batch plot invokes begin generate graphics with nullable argument compatibility' $batchPlotCommands 'SafeInvokeOptionalMethod\(engine,\s*"BeginGenerateGraphics",\s*new\s+object\[\]\s*\{\s*null\s*\}\)'
Assert-Match 'batch plot invokes end generate graphics with nullable argument compatibility' $batchPlotCommands 'SafeInvokeOptionalMethod\(engine,\s*"EndGenerateGraphics",\s*new\s+object\[\]\s*\{\s*null\s*\}\)'
Assert-Match 'batch plot invokes end page with nullable argument compatibility' $batchPlotCommands 'InvokeOptionalMethod\(engine,\s*"EndPage",\s*new\s+object\[\]\s*\{\s*null\s*\}\)'
Assert-Match 'batch plot invokes end document with nullable argument compatibility' $batchPlotCommands 'InvokeOptionalMethod\(engine,\s*"EndDocument",\s*new\s+object\[\]\s*\{\s*null\s*\}\)'
Assert-Match 'batch plot invokes end plot with nullable argument compatibility' $batchPlotCommands 'InvokeOptionalMethod\(engine,\s*"EndPlot",\s*new\s+object\[\]\s*\{\s*null\s*\}\)'
Assert-Match 'api plot saves background plot setting before plotting' $batchPlotCommands 'oldBackgroundPlot\s*=\s*CadApp\.GetSystemVariable\("BACKGROUNDPLOT"\)'
Assert-Match 'api plot disables background plotting during plot' $batchPlotCommands 'CadApp\.SetSystemVariable\("BACKGROUNDPLOT",\s*0\)'
Assert-Match 'api plot restores background plot setting after plotting' $batchPlotCommands 'CadApp\.SetSystemVariable\("BACKGROUNDPLOT",\s*oldBackgroundPlot\)'
Assert-NotMatch 'api plot does not block cad command thread waiting for plot state' $batchPlotCommands 'WaitForNotPlotting'
Assert-Match 'batch plot has optional method helper for old autocad plot engine' $batchPlotCommands 'void\s+InvokeOptionalMethod\s*\('
Assert-Match 'batch plot optional method helper matches method by argument count' $batchPlotCommands 'GetMethods\(BindingFlags\.Public\s*\|\s*BindingFlags\.Instance\)'
...
Assert-Match 'autocad gstarcad and zwcad pdf plot use plot command path' $batchPlotCommands '#if\s+GSTARCAD\s*\|\|\s*ZWCAD\s*\|\|\s*AUTOCAD\s+return\s+PlotFrameToPdfWithPlotCommand\(frame,\s*settings,\s*outputPath\);'
Assert-Match 'autocad gstarcad and zwcad device plot use plot command path' $batchPlotCommands '#if\s+GSTARCAD\s*\|\|\s*ZWCAD\s*\|\|\s*AUTOCAD\s+return\s+PlotFrameToDeviceWithPlotCommand\(frame,\s*settings\);'
Assert-Match 'autocad pdf plot uses plot command path to avoid Plot API WCS window blank output' $batchPlotCommands '#if\s+GSTARCAD\s*\|\|\s*ZWCAD\s*\|\|\s*AUTOCAD\s+return\s+PlotFrameToPdfWithPlotCommand\(frame,\s*settings,\s*outputPath\);'
Assert-Match 'zwcad pdf plot uses plot command path because PlotInfo is unavailable' $batchPlotCommands '#if\s+GSTARCAD\s*\|\|\s*ZWCAD\s*\|\|\s*AUTOCAD\s+return\s+PlotFrameToPdfWithPlotCommand\(frame,\s*settings,\s*outputPath\);'
...
Assert-Match 'api plot resolves plot info from plotting namespace' $batchPlotCommands 'PlotInfoType\s*=\s*RequiredTypeFromCandidates\(plotNs\s*\+\s*"\.PlotInfo"'
Assert-Match 'api plot resolves plot info validator from plotting namespace' $batchPlotCommands 'PlotInfoValidatorType\s*=\s*RequiredTypeFromCandidates\(plotNs\s*\+\s*"\.PlotInfoValidator"'
Assert-NotMatch 'api plot does not resolve plot info from database namespace' $batchPlotCommands 'PlotInfoType\s*=\s*RequiredTypeFromCandidates\(dbNs\s*\+\s*"\.PlotInfo"'
...
Assert-Match 'api plot resolves extents from database namespace first' $batchPlotCommands 'Extents2dType\s*=\s*RequiredTypeFromCandidates\(dbNs\s*\+\s*"\.Extents2d"'
Assert-NotMatch 'api plot does not require extents from geometry namespace first' $batchPlotCommands 'Extents2dType\s*=\s*RequiredTypeFromCandidates\(geoNs\s*\+\s*"\.Extents2d"'
Assert-Match 'api plot keeps geometry extents fallback' $batchPlotCommands 'geoNs\s*\+\s*"\.Extents2d"'
