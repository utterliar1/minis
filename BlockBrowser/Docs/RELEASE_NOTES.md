...
## 2026-07-01：1.3.4 投入使用前稳定性修正

- 修复插入块时部分图纸下指定点不是块基点的问题：插入点和保存块基点统一从当前 UCS 转换到 WCS。
- 修复插入关闭时可能短暂闪到资源管理器等其他窗口的问题，恢复 CAD 模态窗口正常关闭流程。
- 优化缩略图可见区域懒加载：滚动条、鼠标滚轮、尺寸变化和卡片分批追加都会延迟触发当前视口缩略图补载。
- 增强状态诊断，补充保护分类白名单、待同步本地变更状态和同步日志路径。
- 优化构建脚本，优先查找 Visual Studio MSBuild，并在缺少 .NET Framework 4.8 Targeting Pack 时给出明确提示。

## 2026-06-13：1.3.3 缩略图卡片体验修正

- 优化缩略图卡片视觉：减轻边框、加强选中态，并让整张卡片区域都能响应悬停。
- 修复 BB 面板首次未点击分类时，拖动右侧滚动条后左键点击可能回滚到顶部的问题。
- 修复缓存缩略图复用风险，卡片显示使用缓存副本，避免共享图片被释放后出现空白。
- 缩略图名称栏按字体高度动态预留空间，改善 2K 175% 缩放等高 DPI 环境下文字下沿发紧的问题。
- 缩略图生成异常时在卡片上显示轻量失败提示，并在后续成功加载或重置占位图时自动清除。
