...
一个命令 `CC` 呼出分组面板，22 个内置工具 + 无限自定义扩展，配置文件随插件一起迁移。
...
| 规范中心 | `CT_STANDARDCENTER` | 统一进入图层规范、文字规范和配置体检。后续新增规范功能也会优先放在这里。 |
...
运行 `CT_STANDARDCENTER` 可以统一进入图层规范、文字规范和配置体检；也可以运行 `CT_CONFIGCHECK` 或面板左下角齿轮按钮直接检查 `CadToolkit.ini` 是否缺少基础项、官方命令、必要 section，是否存在映射目标缺失或标准行格式错误。
