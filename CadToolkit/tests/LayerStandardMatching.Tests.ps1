...
Assert-Contains 'standard preview helper defines filter controls' $standardPreviewUi 'class\s+StandardPreviewFilterControls'
Assert-Contains 'standard preview helper creates filter controls' $standardPreviewUi 'CreateStandardPreviewFilterControls'
Assert-Contains 'standard preview helper filters tree nodes' $standardPreviewUi 'FilterStandardPreviewNodes'
Assert-Contains 'standard preview helper formats tree report' $standardPreviewUi 'FormatStandardPreviewTreeReport'
Assert-Contains 'standard preview helper updates tree preview' $standardPreviewUi 'UpdateStandardPreviewTree'
...
Assert-Contains 'layer standard preview uses shared form helper' $layerCommands 'CreateStandardPreviewForm'
Assert-Contains 'layer standard preview uses shared filter controls' $layerCommands 'CreateStandardPreviewFilterControls'
Assert-Contains 'layer standard preview uses shared tree helper' $layerCommands 'CreateStandardPreviewTree'
Assert-Contains 'layer standard fallback rebuilds tree preview' $layerCommands 'BuildLayerPlanTreePreview'
Assert-Contains 'standard preview helper has keyword filter box' $standardPreviewUi 'new\s+TextBox\s*\('
Assert-Contains 'standard preview helper has visible search label' $standardPreviewUi 'SearchLabel'
Assert-Contains 'standard preview helper has compact search box' $standardPreviewUi 'Search\.Width\s*=\s*UiScale\(180\)'
Assert-Contains 'layer standard preview rebuilds tree on keyword change' $layerCommands 'TextChanged\s*\+='
Assert-Contains 'standard preview helper expands focused preview tree' $standardPreviewUi 'tree\.ExpandAll\s*\('
