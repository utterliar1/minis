...
Assert-Contains 'create category button matches category button height' $formSource '_btnCreateCategory\s*=\s*new\s+Button[\s\S]*?Size\s*=\s*new\s+Size\(28,\s*24\)'
Assert-Contains 'create category panel centers plus button on layout' $formSource '_catActionPanel\.Layout\s*\+=\s*\(s,\s*e\)\s*=>\s*CenterCreateCategoryButton\(\)'
Assert-Contains 'create category centering helper exists' $formSource 'private\s+void\s+CenterCreateCategoryButton\(\)'
Assert-Contains 'create category button centers horizontally' $formSource '_btnCreateCategory\.Left\s*=\s*Math\.Max\(0,\s*\(_catActionPanel\.ClientSize\.Width\s*-\s*_btnCreateCategory\.Width\)\s*/\s*2\)'
Assert-Contains 'create category button lifts from row center for category visual alignment' $formSource '_btnCreateCategory\.Top\s*=\s*Math\.Max\(0,\s*\(_catActionPanel\.ClientSize\.Height\s*-\s*_btnCreateCategory\.Height\)\s*/\s*2\s*-\s*CreateCategoryButtonVerticalLift\)'
...
Assert-Contains 'create category button cannot draw focus rectangle' $formSource '_btnCreateCategory\s*=\s*new\s+Button[\s\S]*?TabStop\s*=\s*false'
Assert-Contains 'create category button has no heavy border' $formSource '_btnCreateCategory\.FlatAppearance\.BorderSize\s*=\s*0'
Assert-Contains 'create category button border blends into category background' $formSource '_btnCreateCategory\.FlatAppearance\.BorderColor\s*=\s*Color\.FromArgb\(235,\s*238,\s*242\)'
