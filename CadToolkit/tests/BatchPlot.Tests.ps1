...
Assert-Match 'batch plot sorts top to bottom' $batchPlotCommands 'MaxY\.CompareTo\(a\.MaxY\)'
Assert-Match 'batch plot sorts left to right' $batchPlotCommands 'a\.MinX\.CompareTo\(b\.MinX\)'
...
Assert-Literal 'gstarcad plot command uses lisp command wrapper' $batchPlotCommands '"(command "'
...
Assert-Match 'gstarcad plot command wraps send in a quiet progn' $batchPlotCommands 'quietCommandText\s*=\s*"\(progn\s+\(setvar \\"CMDECHO\\" 0\)\s+"\s*\+\s*commandText\.Trim\(\)\s*\+\s*"\s+\(setvar \\"CMDECHO\\" 1\)\s+\(princ\)\)\\n"'
