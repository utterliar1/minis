...
| BBSYNC | 打开同步中心，由指定维护人查看预览后安全同步到 NAS |
| BBTHUMB | 清除缩略图缓存 |
| BBINFO | 显示插件信息 |
...
- **插入** — 选中块后点击插入，或双击缩略图
- **删除 / 重命名** — 右键缩略图后操作（删除需确认）
- **添加到库** — 关闭对话框后在 CAD 中选择对象、指定基点、输入名称
- **导出块** — 从当前图纸批量导出块到库（支持 Ctrl/Shift 多选）
- **设置** — 修改块库路径、插入比例/角度
- **搜索** — 实时过滤块名称（支持模糊匹配）
- **大小** — 切换缩略图尺寸（小/中/大/特大）
- **补全缩略图** — 为当前列表中缺失缓存的块补生成缩略图，不修改 DWG 文件
- **状态诊断** — 查看当前模式、图库路径、NAS/本地副本可访问性、本地变更记录和缩略图缓存数量，可复制给维护人排查
- **打开文件夹** — 在 **图库** 菜单中打开当前图库目录，方便手动查看文件
...
在 `Auto` 模式下，普通同事保持 `AllowNasSync=0`，即使 NAS 在线也优先使用本地副本，NAS 只作为“更新本地图库”的只读来源；如果本地副本还不存在，则可以临时浏览 NAS，但新增、删除、重命名等写入操作会被拦截。指定维护人设置 `AllowNasSync=1` 后，`Auto` 模式才会在 NAS 可用时直接使用 NAS 主图库。
...
`ProtectedLocalCategories` 是本地保护分类白名单，默认 `个人块`。它指的是 `LocalMirrorPath` 下面的顶层分类文件夹，适合放个人常用块、临时块或还不准备共享给团队的内容；多个分类可用分号分隔。
...
- `ProtectedLocalCategories` 中的分类不会被“更新本地图库”覆盖或删除，也不会被 `BBSYNC` 自动同步到 NAS。
- 普通同事删除本地副本里的块时，只删除本地文件，不会写 NAS；下次更新本地图库时，如果 NAS 仍有该块会恢复。
- 指定维护人在本地副本删除块时，会记录为删除请求，不会自动删除 NAS 文件。
- 缩略图缓存仍保留在每台电脑本地，不共享到 NAS。

常用操作：

- 普通同事日常只需要点击面板上的 **更新本地图库**，或运行 `BBMIRROR` 从 NAS 更新本地图库。这个操作会把 NAS 的新增、修改、删除同步到本地；本地已有变更记录的文件会被保护，`个人块` 等保护分类也会被跳过。
- 执行前会先显示树形预览，按新增、覆盖、删除、白名单跳过、本地变更跳过分组列出明细；确认后才会开始执行。
- 更新完成后会显示新增、覆盖、删除、保护跳过总数。预览里的“白名单跳过”来自 `ProtectedLocalCategories`，“本地变更跳过”来自 `.blockbrowser/local-changes.json`，方便判断某个块为什么没被 NAS 覆盖或删除。
- 同步中心只给指定维护人使用。普通电脑保持 `AllowNasSync=0`，维护人电脑改为 `AllowNasSync=1` 后才会显示同步入口并允许执行同步。
- `BBSYNC`：打开同步中心，上传安全的新文件，并汇总重复跳过、白名单跳过、冲突、删除待确认和失败数量；`ProtectedLocalCategories` 中的分类不会自动上传，会在预览和同步中心显示为“白名单跳过”。
...
面板「图库 > 补全缩略图」会为当前筛选结果中还没有缓存的块补生成缩略图；「状态诊断」会汇总当前图库状态、本地变更记录和缩略图缓存数量，适合排查 NAS、同步或缩略图问题时复制给维护人。
